      ...
      <div onClick={onToggleExpand} style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
          <span onClick={(e)=>{e.stopPropagation(); onToggleFavorite?.(recipe.id);}}>
            <Icons.Heart size={19} filled={isFavorite} color={isFavorite?p.accent:"#C9BBA8"} outline={isFavorite?p.deep:"#C9BBA8"} />
          </span>
  ...
  return (
    <div onClick={onToggleExpand} style={{
      ...
      <div style={{ display:"flex", gap:6, marginBottom:11, flexWrap:"wrap" }}>
        ...
        {recipe.protein && <span style={{ background:"#F3EDE3", color:"#8E6F58", fontSize:11, padding:"3px 10px", borderRadius:999 }}>{recipe.protein}g {lang==="en"?"protein":"Protein"}</span>}
              ...
              <li key={i} style={{ fontSize:13.5, color:"#4A3D31", marginBottom:3 }}>
                ...
                {fromFridge && (
                  <span onClick={(e)=>{e.stopPropagation(); onAddSingleIngredient?.(ing.amountOnly, ing.name, recipe);}} style={{
                    marginLeft:8, fontSize:11, color:"#52684A", background:"rgba(110,139,94,0.1)",
      ...
      {recipe.steps?.length > 0 && (
        <div style={{ marginBottom:14 }} onClick={(e)=>e.stopPropagation()}>
          ...
          <ol style={{ margin:0, paddingLeft:18 }}>
            {recipe.steps.map((s,i)=><li key={i} style={{ fontSize:13.5, color:"#4A3D31", marginBottom:5, lineHeight:1.5 }}>{renderBoldText(s)}</li>)}
          </ol>
        </div>
      )}
        ...
        {whyOpen && (
          <div style={{ marginTop:12, background:p.accentSoft, borderRadius:14, padding:14, borderLeft:`3px solid ${p.accent}` }}>
            ...
            {loadingWhy ? <Spinner text="..." /> : (
              <>
                <p style={{ margin:0, fontSize:13, color:"#4A3D31", lineHeight:1.6 }}>{renderBoldText(why)}</p>
                {whySource && <p style={{ margin:"8px 0 0", fontSize:10.5, color:"#B8A48E" }}>{lang==="en"?"Source":"Quelle"}: {whySource}</p>}
              </>
            )}
          </div>
        )}
